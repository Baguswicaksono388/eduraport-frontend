...
<template>
  <div class="space-y-4 animate-in fade-in duration-200">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6" v-if="data">
      ...
      <div class="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-slate-700/60 dark:border-zinc-800/80 rounded-2xl p-5 space-y-4">
        <div class="border-b border-slate-100 dark:border-zinc-800 pb-2 flex justify-between items-center">
          <h4 class="font-bold text-sm text-slate-800 dark:text-slate-200 dark:text-zinc-200">Penggunaan per Komponen</h4>
        ...
        <div class="space-y-2">
          <div v-for="exp in data.expenses" :key="exp.account_code" class="flex justify-between text-xs py-1">
            ...
            <span class="font-mono font-bold text-slate-900 dark:text-slate-100 dark:text-zinc-100">{{ formatNumber(exp.balance) }}</span>
          </div>
        ...
        <div class="flex justify-between border-t border-slate-100 dark:border-zinc-800 pt-3 font-extrabold text-xs">
          <span class="text-slate-800 dark:text-slate-200 dark:text-zinc-200">TOTAL PENGGUNAAN</span>
        ...
        <div class="flex justify-between text-xs py-1">
          ...
          <span class="font-mono font-bold text-slate-900 dark:text-slate-100 dark:text-zinc-100">{{ formatNumber(data.totals.revenue) }}</span>
        </div>
        <div class="flex justify-between border-t border-slate-100 dark:border-zinc-800 pt-3 font-extrabold text-xs">
          <span class="text-slate-800 dark:text-slate-200 dark:text-zinc-200">SALDO BOS</span>
      ...
      <div class="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-slate-700/60 dark:border-zinc-800/80 rounded-2xl p-5 space-y-4">
        <div class="border-b border-slate-100 dark:border-zinc-800 pb-2 flex justify-between items-center">
          <h4 class="font-bold text-sm text-slate-800 dark:text-slate-200 dark:text-zinc-200">Kepatuhan Proporsi Juknis</h4>
        ...
        <div class="space-y-4 mt-4">
          <div class="space-y-1.5">
            <div class="flex justify-between text-xs font-medium">
              <span class="text-slate-700 dark:text-slate-300 dark:text-zinc-300">Perpustakaan (min 10%)</span>
          ...
          <div class="space-y-1.5">
            <div class="flex justify-between text-xs font-medium">
              <span class="text-slate-700 dark:text-slate-300 dark:text-zinc-300">Honor (maks 50%)</span>
        ...
        <p class="text-[11px] text-slate-500 dark:text-slate-400 mt-6 pt-4 border-t border-slate-100 dark:border-zinc-800 leading-relaxed">
