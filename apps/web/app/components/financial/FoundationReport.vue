...
<template>
  <div class="space-y-4 animate-in fade-in duration-200">
    <div class="bg-white dark:bg-zinc-900 border border-slate-200/60 dark:border-zinc-800/80 rounded-2xl overflow-hidden shadow-sm" v-if="data && data.length > 0">
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse text-xs">
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-zinc-800/80 font-medium">
            <tr v-for="school in data" :key="school.school_id" class="hover:bg-slate-50/30 dark:hover:bg-zinc-950/20 text-slate-700 dark:text-zinc-300">
            ...
            <tr class="bg-slate-50/50 dark:bg-zinc-900/50">
              <td class="p-4 pl-6 font-extrabold text-slate-800 dark:text-zinc-200 uppercase">KONSOLIDASI</td>
              <td class="p-4 font-mono font-extrabold text-slate-800 dark:text-zinc-200">{{ formatNumber(totalRevenue) }}</td>
              <td class="p-4 font-mono font-extrabold text-slate-800 dark:text-zinc-200">{{ formatNumber(totalExpense) }}</td>
    ...
    <p class="text-[11px] text-slate-500 mt-2">Sumber: metric_snapshots (7.5) — dashboard yayasan tidak menyentuh tabel transaksional unit.</p>

    <div v-if="!data || data.length === 0" class="py-16 text-center text-slate-400 border border-dashed border-slate-200 dark:border-zinc-800 rounded-xl">
      <p class="text-xs font-bold text-slate-700 dark:text-zinc-300">Belum Ada Data Konsolidasi Yayasan</p>
    </div>
  </div>
</template>
