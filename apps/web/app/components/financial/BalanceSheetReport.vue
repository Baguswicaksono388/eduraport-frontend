...
<template>
  <div class="space-y-4 animate-in fade-in duration-200">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6" v-if="data">
      <!-- Assets Column -->
      <div class="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-slate-700/60 dark:border-zinc-800/80 rounded-2xl p-5 space-y-4 flex flex-col justify-between">
        <div>
          <div class="border-b border-slate-100 dark:border-zinc-800 pb-2 flex justify-between items-center">
            <h4 class="font-bold text-sm text-slate-800 dark:text-slate-200 dark:text-zinc-200">Aset (Assets)</h4>
          ...
          <div class="space-y-2 mt-4">
            <div v-for="acc in data.assets.items" :key="acc.id" class="flex justify-between text-xs py-1">
              ...
              <span class="font-mono font-bold text-slate-900 dark:text-slate-100 dark:text-zinc-100">{{ formatNumber(acc.balance) }}</span>
            </div>
        ...
        <div class="flex justify-between border-t border-slate-100 dark:border-zinc-800 pt-3 font-extrabold text-xs">
          <span class="text-slate-800 dark:text-slate-200 dark:text-zinc-200">TOTAL ASET</span>
          <span class="font-mono">{{ formatNumber(data.assets.total) }}</span>
        </div>
      </div>

      <!-- Liabilities & Equities Column -->
      <div class="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-slate-700/60 dark:border-zinc-800/80 rounded-2xl p-5 space-y-6">
        <!-- Liabilities -->
        <div class="space-y-4">
          <div class="border-b border-slate-100 dark:border-zinc-800 pb-2 flex justify-between items-center">
            <h4 class="font-bold text-sm text-slate-800 dark:text-slate-200 dark:text-zinc-200">Kewajiban (Liabilities)</h4>
          ...
          <div class="space-y-2 mt-4">
            ...
            <div v-if="data.liabilities.items.length === 0" class="flex justify-between text-xs py-1">
              ...
              <span class="font-mono font-bold text-slate-900 dark:text-slate-100 dark:text-zinc-100">Rp 0</span>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-4">
          <div class="border-b border-slate-100 dark:border-zinc-800 pb-2 flex justify-between items-center mt-6">
            <h4 class="font-bold text-sm text-slate-800 dark:text-slate-200 dark:text-zinc-200">Ekuitas (Equity)</h4>
          ...
          <div class="space-y-2 mt-4">
            ...
            <div class="flex justify-between text-xs py-2 px-3 bg-slate-50 dark:bg-slate-800/50 dark:bg-zinc-950 rounded-lg" v-if="Number(data.summary.netIncome) !== 0">
              ...
              <span class="font-mono font-bold text-slate-900 dark:text-slate-100 dark:text-zinc-100">{{ formatNumber(data.summary.netIncome) }}</span>
            </div>
          </div>
        </div>

        <div class="flex justify-between border-t border-slate-100 dark:border-zinc-800 pt-3 font-extrabold text-xs">
          <span class="text-slate-800 dark:text-slate-200 dark:text-zinc-200">TOTAL KEWAJIBAN & EKUITAS</span>
